...
class Calc {
    ...
    fun getInputExpression(): String {
        return inputStack.toArray().joinToString(separator = "")
    }
    ...
    fun pushOperator(operator : String) {
    ...
    fun evalExpression(last: String): String {
        var expr = inputStack.toArray().joinToString(separator = "")
        return try {
            "" + eval(expr)
        } catch (e: Exception) {
            last
